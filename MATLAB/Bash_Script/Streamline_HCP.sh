...
# qsub -terse -j y -pe threaded 1 -l h_vmem=10G -o ${dir_main}/sge_output/streamline/NMF_All_HCP.log ${dir_main}/scripts/HCP_SGC/streamline_All_HCP.sh
####################################################################################
...
dir_result=${dir_main}/Result/${dataset}${subset}/FN_${K}
...
then
    ...
    bash $dir_script/submit_step1_All_HCP.sh -k $K -ds $dataset -ss $subset -log $file_log_main -dlog $dir_log
fi
...
then
    ...
    bash $dir_script/submit_step2_All_HCP.sh -k $K -ds $dataset -ss $subset -log $file_log_main -dlog $dir_log
fi
...
then
    ...
    bash $dir_script/visualize_step2_All_HCP.sh -k $K -ds $dataset -ss $subset -log $file_log_main -dlog $dir_log
fi
...
then
    ...
    bash $dir_script/submit_step3_All_HCP.sh -k $K -ds $dataset -ss $subset -log $file_log_main -dlog $dir_log
fi
...
then
    ...
    bash $dir_script/visualize_step3_All_HCP.sh -k $K -ds $dataset -ss $subset -log $file_log_main -dlog $dir_log
fi
