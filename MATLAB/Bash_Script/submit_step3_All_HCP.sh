...
####################################################################################
# Yuncong Ma, 12/5/2022
# This code is to run individual NMF using Hongming Li's NMF
# Each subject has multiple scans
# bash /cbica/home/mayun/Projects/NMF/scripts/HCP_SGC/submit_step3_All_HCP.sh
####################################################################################
...
do
    ...
    MATLAB_function=deployFuncMvnmfL21p1_func_surf_hcp_single_tp
    MATLAB_bash=${dir_main}/scripts/MATLAB/run_${MATLAB_function}.sh

    jid=$(qsub -terse -j y -pe threaded 1 -l h_vmem=10G -o ${file_log} \
        ${file_run_matlab_compiled} -f ${MATLAB_bash} -p ${sbjFileSingle}'~'${sbjTCFolder}'~'${file_wb}'~'${file_gNb}'~'${dir_out}'~'${resId}'~'${file_init}'~'${K}'~'${alphaS21}'~'${alphaL}'~'${vxI}'~'${spaR}'~'${ard}'~'${eta}'~'${iterNum}'~'${calcGrp}'~'${parforOn}'~'${timepointFile} \
        -log ${file_log})
