...
# qsub -terse -j y -pe threaded 1 -l h_vmem=10G -o ${dir_main}/sge_output/streamline/NMF_All_HCP_2F.log ${dir_main}/scripts/HCP_SGC/streamline_All_HCP_2F.sh
####################################################################################
...
then
    ...
    bash $dir_script/submit_step1_All_HCP_2F.sh -k $K -ds $dataset -ss $subset -nf $N_Fold -f $fold -log $file_log_main -dlog $dir_log
fi
...
then
    ...
    bash $dir_script/submit_step2_All_HCP_2F.sh -k $K -ds $dataset -ss $subset  -nf $N_Fold -f $fold -log $file_log_main -dlog $dir_log
fi
...
then
    ...
    bash $dir_script/visualize_step2_All_HCP_2F.sh -k $K -ds $dataset -ss $subset -nf $N_Fold -f $fold -log $file_log_main -dlog $dir_log
fi
...
then
    ...
    bash $dir_script/submit_step3_All_HCP_2F.sh -k $K -ds $dataset -ss $subset -nf $N_Fold -f $fold -log $file_log_main -dlog $dir_log
fi
...
then
    ...
    bash $dir_script/visualize_step3_All_HCP_2F.sh -k $K -ds $dataset -ss $subset -log $file_log_main -dlog $dir_log
fi
