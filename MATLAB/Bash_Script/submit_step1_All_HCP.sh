...
# bash /cbica/home/mayun/Projects/NMF/scripts/HCP_SGC/submit_step1_All_HCP.sh
####################################################################################
...
for i in $(seq 1 $N_BS)
do
    file_sbjList=${dir_sbjList}/Surface_FIX_2BS100_${i}.txt
    file_timepoint=0
    ...
    #   -log ${file_log})
    jid=$(qsub -terse -j y -pe threaded 1 -l h_vmem=150G -o ${file_log} \
