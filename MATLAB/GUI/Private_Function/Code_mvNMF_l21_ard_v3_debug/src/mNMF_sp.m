function [U_final, V_final, nIter_final, elapse_final, bSuccess, objhistory_final] = mNMF_sp(X, k, W, options, U_, V_)
    ...
    %   Modified by hmli
    %  Modified by Yuncong Ma, 12/12/2022, use litekmeans to initialize U and V
    ...
    Method_Choice='kmeans_Least_Square';
...
%             end
    ...
    if Debug==1
        fprintf('\n--mNMF_sp--\n');
        fprintf('size U_ is %d %d\n',size(U_));
        fprintf('size V_ is %d %d\n',size(V_));
        fprintf('\n');
    end
    %
                ...
                if selectInit
                    ...
                    if isempty(maxIter)
                        ...
                        if isfield(options,'Converge') && options.Converge
                            ...
                            objhistory = [objhistory newobj];
                            if mod(nIter,10)==0
                               disp(['  iter ',num2str(nIter), ' in ',num2str(maxIter)]);
                               disp(newObjStr);
                            end
