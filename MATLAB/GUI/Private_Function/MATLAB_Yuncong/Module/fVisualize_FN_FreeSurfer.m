function fVisualize_FN_FreeSurfer(File_FN,File_Brain_Surface,Dir_Figure,varargin)
% Yuncong Ma, 2/1/2024
% Visualize FN in MGH/MGZ format for NMF APP
% Brain template is based on FreeSurfer
% fVisualize_FN_FreeSurfer(File_FN,File_Brain_Surface,Dir_Figure)
% Work for both group-level and individualized FN

Options.Color_Range_Style='SR-NMF';
Options=fOption('fVisualize_FN_FreeSurfer',Options,varargin);
if isempty(Options)
    return;
end
...
for i=1:k
    ...
    % main figure
    Image=fHCP_FS_6View_Column(Brain_Surface.Shape_Inflated,fHCP_To_32K(squeeze(Center(i,:))',Brain_Surface.MW.LR),...
        Color_Function,{'Material','dull'});
