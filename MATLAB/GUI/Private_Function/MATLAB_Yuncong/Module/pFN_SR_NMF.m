function [Flag,Message]=pFN_SR_NMF(App_Dir,Work_Dir,Subject_Folder)
% Yuncong Ma, 2/1/204
% Compute Personalized FN in each subject folder using SR-NMF
% [Flag,Message]=pFN_SR_NMF(App_Dir,Work_Dir,Subject_Folder)
...
K=FN_Computation.K;
alphaS21=FN_Computation.PersonalizedFN.alphaS21;
alphaL=FN_Computation.PersonalizedFN.alphaL;
vxI=FN_Computation.PersonalizedFN.vxI;
spaR=FN_Computation.PersonalizedFN.spaR;
ard=FN_Computation.PersonalizedFN.ard;
eta=FN_Computation.PersonalizedFN.eta;
iterNum=FN_Computation.PersonalizedFN.iterNum;

% Log file
FID=fopen(fullfile(outDir,'Log.txt'),'w');
fprintf(FID,'pFN_SR_NMF\n Start at %s\n',char(datetime('now')));
...
for si=1:numUsed
    ...
    switch Method_Choice
        ...
        case 'Trim_WA'
            ...
            trimInd = V{si}./max(repmat(miv,size(V{si},1),1),eps) < 5e-2;
            V{si}(trimInd) = 0;
            U{si} = sbjData{si}*V{si}./repmat(sum(V{si},1),[size(sbjData{si},1),1]);
        case 'Random'
            V{si} = initV;
...
save(fullfile(outDir,'Preprocessing.mat'), 'W', 'D', 'L', 'Wt', 'Dt', 'Lt', 'U', 'V', '-v7.3');
