...
% Yuncong Ma, 5/31/2023
% Visualize statistical tests for FN
% fVisualize_Statistics(Folder)
% Options support 'Data_Type' 'Data_Format' 'Folder_Result' 'Flag_Atlas' 'Atlas_Border_Color'
% Visualization with atlas border only works on surface data type

Options.Data_Type='Surface';
Options.Data_Format='HCP_Surface';
Options.Folder_Result='';
Options.Flag_Atlas=0;
Options.Atlas_Border_Color=[1 1 1]*0.1;
...
switch Options.Data_Type
    case 'Surface'
        ...
        if Options.Flag_Atlas>0
            Path_Group_FN=fullfile(Options.Folder_Result,'Group_FN','FN.mat');
            if ~exist(Path_Group_FN,'file')
                msgbox('Cannot find the group FN file','Warning',"Warn");
                return;
            end
            Group_FN=Load_Mathematica_Mat(Path_Group_FN);
            [~,Atlas]=max(Group_FN,[],2);
            Atlas=fHCP_To_32K(Atlas,Brain_Surface.MW.LR);
            Atlas(Atlas==0)=-1;% Set medial wall to -1
        end
        ...
        for i=1:K
            ...
            if Options.Flag_Atlas>0
                ...
                Atlas_k=Atlas;
                Atlas_k(Atlas_k~=i)=-1;% only keep one functional network
                ...
                Image=fHCP_FS_6View_Column(Brain_Surface.Shape_Inflated,fHCP_To_32K(Value(:,i),Brain_Surface.MW.LR),Color_Function,...
                    {'Material','dull'},{'Atlas',Border},{'Atlas_Color',Options.Atlas_Border_Color});
            else
                Image=fHCP_FS_6View_Column(Brain_Surface.Shape_Inflated,fHCP_To_32K(Value(:,i),Brain_Surface.MW.LR),Color_Function,...
                    {'Material','dull'});
            end
