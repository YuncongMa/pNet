function [A,S,Y,numIter,tElapsed,finalResidual]=orthnmfrule_mod(X,kc,kr,option)
...
[inx,C]= litekmeans(X',kc,'Replicates',200); % k-mean clustering, get idx=[1,1,2,2,1,3,3,1,2,3]
Y=(inx*ones(1,kc)-ones(c,1)*cumsum(ones(1,kc)))==0; % obtain logical matrix [1,0,0;1,0,0;0,1,0;0,1,0;1,0,0;0,0,1;...]
Y=Y'+0.2;

[inr,Cr] = litekmeans(X,kr,'Replicates',200);
A = (inr*ones(1,kr)-ones(r,1)*cumsum(ones(1,kr)))==0;
A = A+0.2;
...
for i=1:option.iter
    ...
    if mod(i,10)==0 || i==option.iter
        XfitThis=A*S*Y;
        fitRes = norm(XfitPrevious-XfitThis,'fro');% / norm(XfitPrevious,'fro');
        XfitPrevious=XfitThis;
        curRes = norm(X-XfitThis,'fro');% / norm(X,'fro');
        
        if option.dis
            disp(['Iterating >>>>>> ', num2str(i),'th']);
            s = sprintf('%0.6d, %0.6d', curRes, fitRes);
            disp(s);
        end
