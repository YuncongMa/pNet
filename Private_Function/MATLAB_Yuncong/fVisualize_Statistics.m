...
switch Options.Data_Type
    case 'Surface'
        ...
        Result.T_Value=Result.T_Value.*(Result.P_Value<=Statistics.PValue);
        ...
        for i=1:K
            ...
            Max=double(round(10*prctile(Result.T_Value(:,i),99))/10);
            if Max==0
                Max=double(round(10*max(Result.T_Value(:,i)))/10);
                if Max==0
                    Max=1;
                end
            end
            Color_Function=fColor_Theme('Seed_Map_Positive',[0,0.00001,Max]);
            Image=fHCP_FS_6View_Column(Brain_Surface.Shape_Inflated,fHCP_To_32K(Result.T_Value(:,i),Brain_Surface.MW.LR),Color_Function,...
                {'Material','dull'});
            ...
            fColor_Bar(Color_Function,{500,50,100,30,'w','Arial','bold'},[0,Max],'Horizontal Below Inside');
    ...
    case 'Volume'
        ...
        for i=1:K
            ...
            Map=imresize3(Result.T_Value(:,:,:,i),Upsampling,'nearest');
            ...
            Max=double(round(10*prctile(Result.T_Value(:,:,:,i),99,'all'))/10);
            if Max==0
                Max=double(round(10*max(Result.T_Value(:,:,:,i),[],'all'))/10);
                if Max==0
                    Max=1;
                end
            end
