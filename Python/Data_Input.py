...
def output_FN(FN: np.ndarray or str or tuple,
    ...
    def save_FN(FN_2: np.ndarray, file_output_2: str):
        if dataFormat == 'Volume (*.nii, *.nii.gz, *.mat)':
            ...
            # Create header info
            AX_S = nib.cifti2.BrainModelAxis.from_surface(np.ones(29696), 29696, name='CortexLeft') + \
                   nib.cifti2.BrainModelAxis.from_surface(np.ones(29716), 29716, name='CortexRight')
            AX_T = nib.cifti2.SeriesAxis(start=1, step=1, size=FN_2.shape[1])
            nib.save(nib.Cifti2Image(FN_2, (AX_S, AX_T)), file_output_2)
