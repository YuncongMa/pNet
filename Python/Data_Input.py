...
def compute_brain_template(dataType: str, templateFormat: str,
    ...
    if dataType == 'Volume' and templateFormat == '3D Matrix':
        ...
        Volume_Mask = load_fmri_scan(file_mask_vol, dataType='Volume', dataFormat='Volume (*.nii, *.nii.gz, *.mat)', Reshape=False, Normalization=None)
...
def check_data_type_format(dataType: str,  dataFormat: str, scanFlag=True, logFile=None):
    """
    ...
    if dataType == 'Surface' and dataFormat not in ('HCP Surface (*.cifti, *.mat)', 'MGH Surface (*.mgh)', 'MGZ Surface (*.mgz)', 'FreeSurfer'):
        print_log("When dataType is surface, dataFormat should be one of 'HCP Surface (*.cifti, *.mat)', 'MGH Surface (*.mgh)', 'MGZ Surface (*.mgz)', 'FreeSurfer'",
                  logFile=logFile, stop=True)
        if scanFlag is False and dataFormat != 'FreeSurfer':
            print_log("When the target file contains fMRI data, cannot use dataFormat = 'FreeSurfer'. This is for surface shape files and labels generated by FreeSurfer",
                  logFile=logFile, stop=True)
