...
def gFN_fusion_NCut(gFN_BS, K, NCut_MaxTrial=100, dataPrecision='double', logFile='Log_gFN_fusion_NCut'):
    ...
    corrVal = mat_corr(gFN_BS, dataPrecision)  # similarity between FNs, [K * n_BS, K * n_BS]
    ...
    for ki in range(K):
        if np.sum(C == ki) > 1:
            ...
            corrW = np.abs(mat_corr(candSet, dataPrecision))  # Get the similarity between candidate FNs
